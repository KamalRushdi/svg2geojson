"""Inc 5 v2 checkpoint: simple bbox door/window closing (NO polygonization).

Tests just the door/window closing step — no L-shape splitting, no
polygonization. Each instance becomes a single axis-aligned bounding
rectangle, regardless of whether it's L-shaped.

For comparison, see tests/inc5_v3_checkpoint.py which adds adaptive
multi-leg splitting for windows.

Usage:
    .venv/bin/python -m tests.inc5_v2_checkpoint
"""
...
OUTPUT_DIR = Path("output/inc5_v2")
...
def plot_two_panel(
    ...
    """2-panel: boundary only | boundary + closing rects."""
...
def print_summary(sample_name, closing_result):
    ...
    print(f"  Windows: {closing_result.window_count} instances "
...
def main():
    ...
    # v2: NO L-shape splitting (every instance -> single bbox)
    door_cfg = DoorClosingConfig(split_l_shaped_windows=False)
